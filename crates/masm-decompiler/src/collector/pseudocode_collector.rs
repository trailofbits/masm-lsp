...
use masm_analysis::symbol_resolution::create_resolver;
use masm_analysis::symbol_resolution::SymbolResolver;
use masm_analysis::{
...
impl<'a> PseudocodeCollector<'a> {
    pub fn new(
        ...
        Self {
            ...
            failures: Vec::new(),
            sources,
            resolver: create_resolver(module),
            contracts,
            source_text,
            indent_level: 0,
    ...
    pub(super) fn new_counter(&mut self) -> String {
        let names = crate::ssa::LOOP_COUNTER_NAMES;
    ...
    pub(super) fn compute_output_parametric_expr(
        ...
        let value = state.ctx.get_value(id)?;
        let base_idx = match value.kind {
            crate::ssa::VarKind::Argument(i) => i as i32,
            crate::ssa::VarKind::Local(i) => i as i32,
            crate::ssa::VarKind::Return(i) => i as i32,
        };
        ...
        for (counter_idx, loop_ctx) in self.loop_stack.iter().enumerate().rev() {
            ...
            if loop_ctx.net_effect < 0 {
                ...
                if produced > 0 {
                    if let crate::ssa::VarKind::Local(idx) = value.kind {
    ...
    pub(super) fn dup_template(&mut self, n: usize) -> Option<PseudocodeTemplate> {
        ...
        let mut out = crate::ssa::PseudocodeBuilder::new();
        out.var(var);
        out.text(" = ");
        out.var(src);
        Some(out.build())
                        ...
                        let mut outputs: Vec<SsaId> = current_stack
                            ...
                            .filter(|id| {
                                state.ctx.get_value(**id).map_or(false, |v| match v.kind {
                                    crate::ssa::VarKind::Local(idx) => idx >= local_base,
                                    _ => false,
                                })
                            })
                            .cloned()
