...
use masm_instructions::{
    apply_stack_manipulation, type_of, InstructionType, LocalOp, MemoryOp, StackLike,
    StackLikeResult,
};
...
impl<'a> LocalsAnalyzer<'a> {
    ...
    fn analyze_instruction(
        ...
        let span = inst.span();
        let inst_type = type_of(inst.inner());
        ...
        match inst_type {
            InstructionType::Local(LocalOp::Store) => match inst.inner() {
            ...
            InstructionType::Local(LocalOp::Load) => match inst.inner() {
            ...
            InstructionType::Local(LocalOp::Addr) => {
                if let Instruction::Locaddr(idx) = inst.inner() {
                    if let Some(idx) = u16_imm_to_u16(idx) {
            ...
            InstructionType::Call { dynamic: false } => match inst.inner() {
            ...
            InstructionType::Call { dynamic: true } => {
                // Dynamic calls have unknown stack effects; treat as unknown without assuming a pop.
                addr_stack.clear();
            }
            InstructionType::Memory(MemoryOp::Load) => match inst.inner() {
                Instruction::MemLoad => {
                    addr_stack.pop();
                    addr_stack.push(StackElement::Other);
                }
            ...
            InstructionType::Memory(MemoryOp::Store) => match inst.inner() {
