...
use masm_instructions::{type_of, InstructionType, MemoryOp, StackOp};
...
pub fn actions_for(inst: &Instruction) -> Vec<Action> {
    ...
    let inst_type = type_of(inst);

    // Stack ops: preserve reorder semantics plus counts if known.
    if let InstructionType::Stack(op) = inst_type {
    ...
    if matches!(inst_type, InstructionType::Advice) {
        match inst {
    ...
    // Memory loads/stores
    if let InstructionType::Memory(mem_op) = inst_type {
        match (mem_op, inst) {
            (MemoryOp::Load, Instruction::MemLoad | Instruction::MemLoadImm(_)) => {
                return vec![
                    Action::Pop(1),
                    Action::Push {
                        count: 1,
            ...
            (MemoryOp::Store, Instruction::MemStore | Instruction::MemStoreImm(_)) => {
                return vec![Action::Pop(
                    semantics_of(inst).map(|s| s.pops()).unwrap_or(2),
                )];
            }
            (
                MemoryOp::Store,
                ...
                return vec![Action::Pop(
                    semantics_of(inst).map(|s| s.pops()).unwrap_or(5),
                )];
            }
            _ => {}
        }
    ...
    // Merkle ops
    if matches!(inst_type, InstructionType::Merkle) {
        match inst {
            ...
            Instruction::MTreeVerify | Instruction::MTreeVerifyWithError(_) => {
                return vec![Action::Pop(
                    semantics_of(inst).map(|s| s.pops()).unwrap_or(0),
                )];
            }
            _ => {}
        }
    ...
    // Literal pushes
    if matches!(inst_type, InstructionType::Push) {
        match inst {
            Instruction::Push(_) => {
                return vec![Action::Push {
                    count: 1,
